function [idx, C, cost] = spd_kmeans(input, K, initlabel)

% SPD_KMEANS is an adaptation of classical k-means algorithm to
...
%
%   * USAGE
%       [idx, C, cost] = spd_kmeans(input, K)
%       [idx, C, cost] = spd_kmeans(input, K, initlabel)
%   * INPUT
...
if (~spdaux_checker(input))
    error('* spd_kmeans : incorrect input. Please use SPD_INITIALIZE before usage.');
end
...
K = round(K);
if ((K<=1)||(K>=N))
    error('* spd_kmeans : K should be a number in (1,#{data}).');
end
%   3. initlabel
if (nargin < 3)
    if (rand()<0.5) % 50% chance of random selection
        ...
        flagger = true;
        while (flagger)
            initlabel = spd_kmeans_initialization(input, K);
            flagger   = (~(length(unique(initlabel))==K));
        end
    end
    initlabel = spd_kmeans_initialization(input, K);
...
[old_means, old_variation] = spd_kmeans_perclass(input, old_label); % oh
old_cost  = spd_kmeans_cost(old_variation, old_label);
...
while (increment > par_incre)
    ...
    pdist2    = spdaux_pdist2(old_means, input.data);
    ...
    %   2. update
    [new_means, new_variation] = spd_kmeans_perclass(input, new_label);
    new_cost = spd_kmeans_cost(new_variation, new_label);
...
%% auxiliary functions
%   1. equivariant embedding + kmeans++ for initialization
function label = spd_kmeans_initialization(input, K)
    p = input.size(1);
    N = input.size(3);
    ...
    label = kmeans(mydata,K);
end
%   2. compute means for each class using 'spdaux_mean3d'
function [centers, variation] = spd_kmeans_perclass(input, label)
    ...
    for i=1:K % main iteration
        idx = (label==ulabel(i));
        [part_mat, part_var] = spdaux_mean3d(input.data(:,:,idx));
        centers(:,:,i) = part_mat;
        variation(i)   = part_var;
    end
end
%   3. compute cost function given variation vector and class label
function cost = spd_kmeans_cost(variation, label)
    ulabel = sort(unique(label));
    K      = length(ulabel);

...
aux_pdist_WL <- function(means, concentrations){
  n = base::nrow(means)
  d = base::ncol(means)
  ...
  for (i in 1:(n-1)){
    for (j in (i+1):n){
      d1 = (acos(sum(as.vector(means[i,])*as.vector(means[j,])))^2)
      d2 = (d-1)*((1/sqrt(concentrations[i]) - 1/sqrt(concentrations[j]))^2)
      
      output[i,j] <- output[j,i] <- sqrt(scale*d1 + (2-scale)*d2)
    }
  }
  return(output)
}
